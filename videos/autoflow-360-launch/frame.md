...
description: >
  ...
  accent, the three-step gray text ladder, NotoSansSC (display/numerals/chrome) + NotoSansSC (body),
...
colors:
  ...
  text-muted: "#667085"
  text-light: "#98a2b3"
  accent-light: "rgba(37, 99, 184, 0.10)"
  accent-medium: "rgba(37, 99, 184, 0.18)"
  border: "rgba(37, 99, 184, 0.24)"
  card-bg: "rgba(37, 99, 184, 0.05)"
  positive: "#059669"
  negative: "#dc2626"
...
typography:
  # — reading ramp (local Noto Sans SC variable font for offline CJK rendering) —
  body:    { fontFamily: "NotoSansSC", cqw: 0.85, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "NotoSansSC", cqw: 0.8, weight: 600, tracking: "0.08em", upper: true, color: "primary" }
  tag:     { fontFamily: "NotoSansSC", px: 12, weight: 500, color: "primary" }
  counter: { fontFamily: "NotoSansSC", px: 13, weight: 500, tracking: "0.05em", color: "text-muted" }
  # — display / numerical ramp (NotoSansSC, near-black headings / cobalt numerals) —
  h3:      { fontFamily: "NotoSansSC", cqw: 1.25, weight: 500, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  stat-num:{ fontFamily: "NotoSansSC", cqw: 1.9, weight: 700, lineHeight: 1.0, color: "primary" }
  blockquote:{ fontFamily: "NotoSansSC", cqw: 2.4, weight: 500, lineHeight: 1.35, color: "text" }
  h2:      { fontFamily: "NotoSansSC", cqw: 2.6, weight: 600, lineHeight: 1.1, tracking: "-0.02em", color: "text" }
  metric-value:{ fontFamily: "NotoSansSC", cqw: 3.0, weight: 700, lineHeight: 1.0, color: "primary" }
  h1:      { fontFamily: "NotoSansSC", cqw: 4.2, weight: 700, lineHeight: 1.08, tracking: "-0.02em", color: "text" }
  quote-mark:{ fontFamily: "NotoSansSC", cqw: 8.0, weight: 700, lineHeight: 0.5, color: "primary", opacity: 0.15 }
...
This is the **blue-professional** preset remixed onto the AutoFlow 360 product brand. The YAML frontmatter above (colors · typography · components) is **normative and already correct — use it verbatim.** Color values come from the product's own `--af-*` CSS tokens and logo, not from the generic login-page status palette. The prose below is the ORIGINAL preset's intent; read it THROUGH the frontmatter:

- **Fonts** — already set to the locally shipped **NotoSansSC variable font** for display, body, numerals, and Chinese text; ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{100, 200, 300, 400, 420, 500, 600, 700, 800, 900}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
The voice is one locally shipped variable face in fixed roles: **NotoSansSC** (display, every numeral,
all chrome, and body — eyebrows uppercase 0.08em; body muted gray, line 1.6). Headlines are near-black;
...
- **NotoSansSC** (display/numerals/chrome/body) — near-black headlines, cobalt numerals, and deterministic offline Chinese glyphs.
...
Two ramps. The **reading ramp** (NotoSansSC body 0.85cqw muted; NotoSansSC eyebrow uppercase 0.08em
cobalt) carries copy + chrome; the **display/numerical ramp** (NotoSansSC `h3` 1.25cqw → `h1`
...
- **Headlines near-black, −0.02em**; **eyebrows cobalt, uppercase, 0.08em**; **numerals cobalt 600–700**; **body NotoSansSC 400 muted, line 1.6**. No italic, no uppercase body, no cobalt headline.
...
**Composes** accent-line, meta, h1, body sub. **Focal** a 2-line NotoSansSC `h1` near-black, left,
...
**Focal** a row of tinted cards — cobalt `metric-value` + NotoSansSC label + muted desc + optional
...
blockquote, cite. **Focal** a NotoSansSC `blockquote` near-black under a 15%-opacity cobalt
...
NotoSansSC body column beside a cobalt-8% highlight block (4px cobalt left rule) carrying an inline pull
...
a NotoSansSC `h1` near-black, centered, with the one solid cobalt `cta-button` pill below. **Accent**
...
- Use **tinted cards** (4% fill, 20% border, 10–14px radius, no shadow); body NotoSansSC 400 muted, line 1.6.
...
- **Type** — NotoSansSC headings −0.02em near-black, cobalt eyebrows 0.08em + numerals; NotoSansSC body muted line 1.6; ≥1.4cqw floor.
...
- **NotoSansSC ships locally at `assets/fonts/NotoSansSC-VF.ttf`.** Every frame must declare it with `@font-face`; no Google Fonts or other network font is allowed. The eyebrow's uppercase+tracking signal weakens in CJK — pair it with the accent-line.
