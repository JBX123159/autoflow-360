...
| CI、多架构镜像、免费部署 | `.github/workflows/`、`deploy/oracle/` | 171 项静态契约、远端完整集成、多架构构建、Bash/Compose 解析 | 部分通过 | 静态、完整集成和 amd64/arm64 镜像已远端通过；Oracle 云主机尚未实跑 |
...
静态、完整集成和 `v1.0.0-rc2` 多架构镜像 GitHub Actions 已通过；演示视频 Release 附件可匿名下载。临时或 Oracle 演示地址仍未部署，远端业务应用可达性未满足前保持“发布候选”而不是正式生产发布。
