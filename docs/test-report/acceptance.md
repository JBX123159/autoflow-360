...
软件验收日期：2026-08-01；视频成片更新：2026-08-08。数据边界：全部为合成演示/性能数据，默认币种 CNY；结果不代表真实企业采用、用户规模、营收或经营成效。

## 结论

核心业务代码、权限、AI 安全边界、角色工作台、三条演示数据和浏览器流程已经在本地真实 Frappe/ERPNext/CRM 环境验证。静态契约 170/170、完整 Frappe 业务回归 148/148、三条 Playwright 流程 3/3。数据库、公有附件、私有附件和站点配置完成 SHA-256 校验，并成功恢复到一次性隔离站点，取得 `RESTORE_CHECK_PASSED`。9 张桌面/移动端业务截图已从本机真实站点自动采集并逐张验收；演示视频包含 14 个动画分镜、14 段配音、62 组字幕和 13 个转场，运行、布局、动画、字幕安全区、图片越界与文字对比度门禁均为 0 错误。完整预览获人工确认后已渲染 162.633 秒高质量 MP4，并通过完整解码、黑帧、长静音、响度、抽帧总览和尾帧检查。远端 GitHub Actions、多架构镜像和公开访问仍未完成，因此当前仍是本地发布候选，不描述为已上线生产系统。
...
| CI、多架构镜像、免费部署 | `.github/workflows/`、`deploy/oracle/` | 170 项静态契约、Bash/Compose 解析 | 待远端 | 本地配置通过；尚未推送触发 GitHub Actions/构建镜像 |
...
| 测试层 | 实际结果 | 说明 |
| --- | --- | --- |
| 静态契约 | 170/170 | 包含元数据、业务、安全、性能与交付物合同 |
| 完整 Frappe 集成回归 | 148/148 | 当前工作树合并后全量回归 |
| 演示种子定向回归 | 7/7 | 包含重复生成后重新置顶的可见性测试 |
| 浏览器闭环 | 3/3 | 正常交付、供应商延期、重新打样；可见性修复后复测 |
| npm audit | 0 个已知漏洞 | Playwright 锁定安装时的结果 |
| 恢复演练 | 通过 | SHA-256 校验、隔离站点恢复、迁移、应用和 DocType 读取 |

## 性能基线

环境：WSL2、16 CPU、7810 MB 内存、Python 3.14.2、Frappe 16.29.0、ERPNext 16.30.0、CRM 1.80.0、MariaDB 11.8.8。每项先预热 1 次，再记录 10 次，P95 使用 nearest-rank。原始记录见 `performance.json`。

| 操作 | 数据规模/结果 | P50 | P95 |
| --- | --- | ---: | ---: |
| 工作台项目列表 | 返回 50 条 | 37.027 ms | 42.9 ms |
| 项目全景详情 | 单项目聚合 | 16.623 ms | 17.683 ms |
| 每日风险扫描 | 扫描 203 个项目 | 4556.394 ms | 5888.688 ms |
| 每周 AI 草稿调度 | AI 关闭，返回 0 | 0.457 ms | 0.6 ms |

性能数据集包含 200 项目、1000 样品、1000 反馈、500 销售订单、500 采购订单，以及合计 5000 条风险/异常/版本记录。它仅用于本机基线；没有并发压测、云网络延迟或生产 SLA 结论。

## 发布门禁

在推送、合并和标记 `v1.0.0-rc1` 前仍须补齐：GitHub Actions 绿灯、多架构镜像成功，以及临时或 Oracle 演示地址外部可达。演示视频人工确认、最终 MP4 渲染、成片视听验收、截图、本地全量业务回归、三条 E2E 和恢复演练已经完成；远端门禁未满足前保持“发布候选”而不是正式生产发布。
