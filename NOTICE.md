# Third-Party Notices

AutoFlow 360 是独立自定义应用，不是 Frappe、ERPNext 或 Frappe CRM 的官方产品。

| 项目 | 用途 | 来源 | 许可证 |
| --- | --- | --- | --- |
| Frappe Framework | 应用框架 | https://github.com/frappe/frappe | MIT |
| ERPNext | 销售、采购、库存、财务 | https://github.com/frappe/erpnext | GPL-3.0 |
| Frappe CRM | 线索、组织、联系人、商机 | https://github.com/frappe/crm | AGPL-3.0 |
| frappe_docker | 容器构建与部署参考 | https://github.com/frappe/frappe_docker | MIT |
| Noto Sans SC Variable Font | 视频离线中文字体 | https://github.com/notofonts/noto-cjk | SIL Open Font License 1.1（许可证信息同时内嵌于字体文件） |
| GSAP 3.15.0 | 视频离线时间线动画 | https://gsap.com | GSAP Standard License；保留原文件头及其许可证链接 |

本仓库自主新增范围包括客户项目、样品闭环、风险引擎、异常整改、
客户/供应商门户扩展、AI 分析审计、演示数据、自动化测试和求职交付材料；
当前状态以实际代码、测试与验收报告为准。

各上游组件仍由其原作者拥有版权，并适用各自仓库中的许可证和通知文件。本通知不改变任何上游许可证，也不表示上游项目对 AutoFlow 360 的认可或背书。
