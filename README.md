...
截至 2026-07-31，仓库已经完成基础权限与跨单据项目字段、CRM 商机转客户项目、项目阶段机、样品检验/发货/客户反馈/重新打样、报价门槛、不可冒用身份的额度审批、报价转销售订单，以及销售订单驱动的可解释物料缺口与幂等采购需求。当前本地真实 Frappe/ERPNext/CRM 环境通过 **50 项集成测试**与 **87 项静态契约测试**；供应商询报价、库存交付、财务、风险和结项模块仍按实施计划继续开发。
...
- **已验证的本仓库新增范围：** 汽车客户项目、样品闭环、不可冒用身份的报价审批、报价转销售订单，以及可解释物料缺口和采购需求；完成状态以本页里程碑与测试结果为准。
- **规划中的自主新增范围：** 供应商询报价与交期协同、库存交付、确定性风险引擎、异常整改、角色权限矩阵、供应商门户扩展、可审计 AI 辅助、演示数据和完整交付材料。上述范围计划自主实现，当前状态以实际代码和测试为准。
